...
export default async function SubcategoryPage({ params }: PageProps) {
	...
	const parentCategory = await db
		.select()
		.from(categories)
		.where(and(eq(categories.slug, parentSlug), eq(categories.gender, gender), isNull(categories.parentId)))
		.get()
	...
	const subcategory = await db
		.select()
		.from(categories)
		.where(and(eq(categories.slug, slug), eq(categories.gender, gender), eq(categories.parentId, parentCategory.id)))
		.get()
...
export async function generateStaticParams() {
	...
	for (const parent of parents) {
		const children = await db.select({ slug: categories.slug }).from(categories).where(eq(categories.parentId, parent.id)).all()
